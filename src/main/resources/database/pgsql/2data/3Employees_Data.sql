...
USING (
      ...
      ) AS S (employee_id, last_name, first_name, birth_date, photo, notes)
    ON T.employee_id = S.employee_id
WHEN MATCHED AND (T.last_name IS DISTINCT FROM S.last_name) OR (T.first_name IS DISTINCT FROM S.first_name) OR (T.birth_date IS DISTINCT FROM S.birth_date) OR (T.photo IS DISTINCT FROM S.photo) OR (T.notes IS DISTINCT FROM S.notes)
    THEN UPDATE SET last_name = S.last_name, first_name = S.first_name, birth_date = S.birth_date, photo = S.photo, notes = S.notes
WHEN NOT MATCHED
    THEN INSERT (employee_id, last_name, first_name, birth_date, photo, notes)
         OVERRIDING SYSTEM VALUE
         VALUES (S.employee_id, S.last_name, S.first_name, S.birth_date, S.photo, S.notes)
;
SELECT setval('public.employees_employee_id_seq', (SELECT MAX(employee_id) FROM public.employees));