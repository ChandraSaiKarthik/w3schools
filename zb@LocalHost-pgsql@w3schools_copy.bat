...
set "pgsqltarget=C:\zProjects\w3schools_copy-local\src\main\resources\database\pgsql"
...
echo Replacing "w3schools" with "w3schools_copy" in fill_in_placeholders.sh file...
set "targetfile=%pgsqltarget:C:\=/c/%/fill_in_placeholders.sh"
"C:\Program Files\Git\bin\sh.exe" -c ^"sed -i \"s|w3schools|w3schools_copy|g\" \"%targetfile%\""
...
echo Creating Database "w3schools_copy" if not exists...
set "PGPASSWORD=asdf1234"
set "PATH=%PATH%;C:\Program Files\PostgreSQL\16\bin"
psql -h "localhost" -p "5432" -U "postgres" -d "postgres" -f "0CreateDatabase.txt" >nul 2>&1
echo.
...
python pypgsql_deploy.py --DB_HostName="localhost" --DB_Port="5432" --DB_UserName="postgres" --DB_Password="asdf1234" --DB_Name="w3schools_copy"
echo.
